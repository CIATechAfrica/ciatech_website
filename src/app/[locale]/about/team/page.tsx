...
export default async function TeamPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const heroData = {
    ...
    description: "Meet the brilliant minds powering CIATECH Africa's mission to transform the continent.",
    imageRef: "https://images.unsplash.com/photo-1522071820081-009f0129c71c?q=80&w=2070&auto=format&fit=crop",
    ctas: []
  };
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {teamMembers.map((member: any) => (
            <div key={member._id} className="group flex flex-col items-center">
              ...
              <p className="text-sm font-medium text-primary text-center uppercase tracking-wider">{member.role}</p>
            </div>
          ))}
        </div>
      </div>
