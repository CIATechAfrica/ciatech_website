...
export default function GalleryPage() {
  ...
  const activeImage = activeIndex !== null ? galleryData.images[activeIndex] : null;

  const showNext = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (activeIndex !== null) {
      setActiveIndex(activeIndex === galleryData.images.length - 1 ? 0 : activeIndex + 1);
    }
  };
  ...
  const showPrev = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (activeIndex !== null) {
      setActiveIndex(activeIndex === 0 ? galleryData.images.length - 1 : activeIndex - 1);
    }
  };
        ...
        <div className="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[300px]">
            {galleryData.images.map((item, index) => (
              <div 
                key={item.id} 
                className={`group relative rounded-[2rem] overflow-hidden cursor-pointer shadow-sm hover:shadow-2xl transition-all duration-700 hover:-translate-y-2 border border-gray-100 bg-white ${item.span || 'col-span-1'}`}
