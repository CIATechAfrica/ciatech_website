...
export default function ResearchPage() {
  const featured = researchData.featured;
  const [readingItem, setReadingItem] = useState<ResearchItem | null>(null);

  // Dynamic Client-Side PDF Generation
  const handleDownload = async (e: React.MouseEvent, item: ResearchItem) => {
    e.preventDefault();
    e.stopPropagation();
    try {
    ...
    <main className="min-h-screen bg-gray-50 pt-24 font-sans selection:bg-secondary/30 selection:text-gray-900 overflow-x-hidden relative">
      ...
      <section className="relative overflow-hidden bg-[#050505] py-24 sm:py-32">
        <div className="absolute inset-0">
          <Image 
            src={researchData.hero.imageRef} 
            alt="Research Hero" 
            fill 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-black text-white tracking-tight mb-8 leading-tight break-words hyphens-auto">
            {researchData.hero.heading}
          </h1>
          <p className="text-xl md:text-2xl text-gray-400 font-light max-w-3xl mx-auto leading-relaxed">
            {researchData.hero.subtext}
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between border-b border-gray-200 pb-8 mb-16">
            <h2 className="text-3xl font-black text-gray-900 tracking-tight">
              {researchData.publicationsTitle}
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {researchData.publications.map((item) => (
              <div key={item.id} className="group relative bg-white rounded-3xl p-8 border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-2 flex flex-col h-full">
